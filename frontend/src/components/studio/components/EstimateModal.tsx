...
import { Clock, DollarSign, Film, ChevronDown, ChevronRight } from 'lucide-react';
...
export function EstimateModal({ open, onOpenChange, estimateData }: EstimateModalProps) {
  const [showPerVideo, setShowPerVideo] = useState(false);

  return (
    <Dialog open={open && estimateData !== null} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-7xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="text-xs text-muted-foreground pt-2">
            ...
            <p className="mt-1">* Cost estimates are based on the selected model's pricing</p>
          </div>
        </div>
        )}
      </DialogContent>
